...
const staticData = [
  { name: "Short-term rentals", icon: Calendar, color: "#3b82f6", value: 28500 },
  { name: "Long-term leases", icon: Key, color: "#10b981", value: 22000 },
  { name: "Property fees", icon: Home, color: "#8b5cf6", value: 18750 },
  { name: "Insurance", icon: Shield, color: "#f59e0b", value: 15200 },
]
...
export function AnimatedRevenueChart() {
  ...
  // Animate data changes only after mounting
  useEffect(() => {
    if (!mounted) return
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % staticData.length)
    }, 2000)
    return () => clearInterval(interval)
  }, [mounted])
